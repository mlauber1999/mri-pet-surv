...
def sur_loss(preds, obss, hits, bins=Tensor([[0, 12, 24, 48,
                                                          108]])):
...
def cox_loss_orig(risk_pred, y, e):
    risk_pred = risk_pred.view(-1,1)
    y = y.view(-1,1)
    e = e.view(-1,1)
    mask = torch.ones(y.shape[0], y.shape[0])
    mask[(y.T - y) > 0] = 0
    exp_loss = torch.exp(risk_pred) * mask
    sum_exp_loss = torch.sum(exp_loss, dim=0)
    log_loss = torch.log(sum_exp_loss)
    ...
    # get unique rows
    # get non zero values
    # compute nchoosek(exp_loss_values, # hits at this time point)
    # take log
    # subtract
