...
parfor i=1:length(rids)
        rand('state',10);
        disp(['rsync -av ' MRI_folder_old rids{i} '_mri.nii ' new_mri_dir filesep])
        system(['rsync -av ' MRI_folder_old rids{i} '_mri.nii ' new_mri_dir filesep]);
...
% cfg_basicio BasicIO - Unknown
%-----------------------------------------------------------------------
matlabbatch{1}.spm.spatial.preproc.channel.vols = {[old_dir '/' rid '_mri.nii,1']};
...
matlabbatch{3}.spm.spatial.coreg.estwrite.ref = {'./metadata/mri-pet-data-raw/mni152/mni_brain.nii,1'};
