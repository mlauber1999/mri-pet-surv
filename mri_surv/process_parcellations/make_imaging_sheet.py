...
def process_subj(rid: int, path: str, statspath: str, total_subjs: int, pos: int) -> Dict:
    ...
    # labels, data = zip(*csf)
    # labels = [f'csf_vol_{lab}' for lab in labels]
    # csf = dict(zip(labels, data))
    #
    # corr_csf_vollabs = [f'corr_{lab}' for lab in labels]
    # corr_csf_vol = [float(val/tiv) for val in data]
    # corr_csf_vol = dict(zip(corr_csf_vollabs, corr_csf_vol))
    #
    # # cortical thickness
    # xmlreader.filename = f'{path}/catROIs_{rid}_mri.xml' if 'mri' not in rid else f'{path}/catROIs_{rid}.xml'
    #
    # cort_thick = xmlreader.parseXML('aparc_DK40', 'thickness')
    # labels, data = zip(*cort_thick)
    # labels = [f'surf_{lab}' for lab in labels]
    # cort_thick = dict(zip(labels, data))
    #
    # corr_thicklabs = [f'corr_{lab}' for lab in labels]
    # corr_thick = [float(val/tsa) for val in data]
    # corr_thick = dict(zip(corr_thicklabs, corr_thick))
    #
    # # create merged dictionary
    # merged_data = {'RID': rid, 'TIV': tiv, 'TSA': tsa,
    #                **subcort, **cort_thick, **csf, **corr_vol, **corr_thick,
    #                **corr_csf_vol}
    merged_data = {'RID': rid, 'TIV': tiv, 'TSA': tsa,
                   **corr_vol}
    return merged_data
