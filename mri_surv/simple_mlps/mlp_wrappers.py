...
from simple_mlps.loss_functions import sur_loss
from simple_mlps.datas import ParcellationData, \
    ParcellationDataNacc, ParcellationDataVentricles, \
    ParcellationDataVentriclesNacc, ParcellationDataCSF
...
class MLP_Wrapper_Meta:
    def __init__(self, exp_idx,
                 ...
                 dataset_kwargs,
                 criterion, dataset=ParcellationData,
    ...
    def get_c_index_external(self, concordance_time=24):
        ...
        for mlp in self.mlps:
            c_idx, brier_scores, = mlp.test_surv_data_optimal_epoch(
                    self.bins, concordance_time,external_data=True)
...
def main(force=False):
    sur = SurLossUtility('mlp_parcellation_ventricles_sur_loss')
    ...
    # csfsur = SurLossUtility('mlp_csf_sur_loss')
    # csfsur.train(force=force)
    # csfsur.get_c_index()
    # csfsur.write_c_index()

    # agesur = SurLossUtility('mlp_parcellation_ventricles_sur_loss_age')
    # agesur.train(force=force)
    # agesur.get_c_index()
    # agesur.write_c_index()

    # mmsesur = SurLossUtility('mlp_parcellation_ventricles_sur_loss_mmse')
    # mmsesur.train(force=force)
    # mmsesur.get_c_index()
    # mmsesur.write_c_index()

    # mmsesur = SurLossUtility('mlp_parcellation_ventricles_sur_loss_age_mmse')
    # mmsesur.train(force=force)
    # mmsesur.get_c_index()
    # mmsesur.write_c_index()