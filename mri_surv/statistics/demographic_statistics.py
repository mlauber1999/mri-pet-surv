from statistics.utilities import load_json, deabbreviate_parcellation, upper_and_lower_quartiles
...
with open(logger_name,'w') as fi: fi.write('')
...
class _DemographicStatistics(object):
    def __init__(self, dataset='ADNI'):
        self.__assign_props(
                json_fi=JSON_FI, dataset=dataset
        )
    ...
    def __recode_progression_categories(self):
        self.df['PROGRESSION_CATEGORY'] = \
            self.df['PROGRESSION_CATEGORY'].map({
                2: '\u003C4 years',
                1: '\u003C2 years',
                -1: '\u22654 years'
        })
        self.df['PROGRESSION_CATEGORY'] = \
            self.df['PROGRESSION_CATEGORY'].fillna('Censored')
        self.df['PROGRESSION_CATEGORY'] = \
            self.df['PROGRESSION_CATEGORY'].astype('category')
        self.df['PROGRESSION_CATEGORY'] = \
            self.df['PROGRESSION_CATEGORY'].cat.reorder_categories([
                    '\u003C2 years',
                    '\u003C4 years',
                    '\u22654 years',
                    'Censored'
    ...
    def __recode_categorical_values(self):
        for col in self.demographics_columns['categorical']:
            self.df[col] = [str(int(x)) if isinstance(x,float) and not
                    np.isnan(x) else x for x in self.df[col]]
            self.df[col] = self.df[col].astype('category')

    def __cross_tabulate_columns(self):
        self.df_crosstab = [
            pd.crosstab(
                self.df['PROGRESSION_CATEGORY'], self.df[col], margins=True,
                )
            for col in self.demographics_columns['categorical']
            ]
    ...
    def __summarize_continuous_columns_and_values(self):
        ...
        for col in self.demographics_columns['continuous']:
            ...
            self.df_continuous[self.column_maps[col]] = \
                df.groupby('PROGRESSION_CATEGORY').describe()
            self.df_continuous[self.column_maps[col]] = self.df_continuous[self.column_maps[col]].T
            self.df_continuous[self.column_maps[col]].index = \
                self.df_continuous[self.column_maps[col]].index.droplevel(level=0)
    ...
    def write_summary_stats(self):
        for f in self.df_crosstab:
            ...
            f.to_csv(
                self.output_file, sep=',', index=True, mode='a'
            )
        for key in self.df_continuous:
            with open(self.output_file, 'a') as fi:
                fi.write(f'\n'+'-'*30 + f'\n{key}\n')
                fi.write(tabulate(self.df_continuous[key], \
                    headers=self.df_continuous[key].columns))
                fi.write(f'\n\n')

class TrainAndTestStats(object):
    def __init__(self, train_stats: _DemographicStatistics = None, test_stats:
            _DemographicStatistics = None):
        ...
        self.merged_df = _merge_df(
                train_stats, test_stats
        )
        self.merged_df_parcellations = self.merged_df.merge(
                self.parcellation, left_on=['RID'], right_on=[
        'RID'])
    ...
    def _load_parcellations(self):
        self.parcellation = pd.read_csv(
                self.props['parcellation_csv']
        )
        self.parcellation.drop(columns=self.props['ventricles']+[
                'Dataset'] + ['PROGRESSION_CATEGORY'], inplace=True)
    ...
    def _cross_tabulate_columns(self):
        ...
        for col in self.crosstab_columns:
            ...
            df_crosstab_normalized[col].drop(columns=['All'], index=['All'],
                                    inplace=True)
            logger.info(col)
            logger.info(df_crosstab)
            logger.info(f'Total length: {len(self.merged_df[col])}-Total nans: {np.sum(pd.isna(self.merged_df[col]))}')
    ...
    def _create_subplot(self, df, fig, ax, strip=False):
        if strip:
           sns.swarmplot(x="Time to progress to AD", y='Values_Test',
             hue='Dataset',
             dodge=True,
             size=5,
             data=df,
             ax=ax)
           sns.violinplot(x="Time to progress to AD", y='Values_Train',
                          hue='Dataset',
                          split=True,
                          inner="quartile",
                          data=df,
           ...
           sns.despine(fig=fig, ax=ax,
            ...
            sns.violinplot(x="Time to progress to AD", y='Value',
               hue='Dataset',
               split=True,
               inner="quartile",
               data=df,
    ...
    def generate_stacked_bar(self, columns=None):
        ...
        for i, col in enumerate(columns):
            fig, ax = plt.subplots()
            self.df_crosstab_normalized[col].T.plot.bar(stacked=False,
                                                     ax=ax)
            ax.set_xlabel(col)
            ...
            plt.savefig(
                    ...
                    f'{str(i).zfill(2)}' + '.svg', dpi=300)
            plt.savefig(
                    f'figures/figure_1/demographic_statistics_stacked_bar_'
                    f'{str(i).zfill(2)}' + '.png', dpi=300)
            plt.close()

    def generate_violin_plots(self):
        df = _get_melted_df(
                self.merged_df, self.shared_columns,
        ...
        for i, col in enumerate(
            ...
            plt.savefig(f'figures/figure_1/demographic_statistics_violin_plots_'
                        f'{str(i).zfill(2)}' +
                        '.svg', dpi=300)
            plt.savefig(f'figures/figure_1/demographic_statistics_violin_plots_'
                        f'{str(i).zfill(2)}' +
    ...
    def compare_surv_different_times(self, time):
        ...
        self._nacc_mdl = KaplanMeierFitter().fit(self.km_dataset['NACC'][
                                                     'TIMES'],
                                                 self.km_dataset['NACC'][
                                                     'PROGRESSES'],
                                                 label='NACC')
        self._adni_mdl = KaplanMeierFitter().fit(self.km_dataset['ADNI'][
                                                     'TIMES'],
                                                 self.km_dataset['ADNI'][
                                                     'PROGRESSES'],
                                                 label='ADNI')
        with open(self.stats_output, 'a') as fi:
            fi.write('-----------------\nComparing NACC and ADNI at different '
                     'times\n')
            for t in time:
                sA_t = self._adni_mdl.predict(t)
                sB_t = self._nacc_mdl.predict(t)
                results = surv_fixed_point(t, self._adni_mdl,
                                               self._nacc_mdl, point_estimate=f'log(-log(x)) difference ' + \
                                             f'ADNI-NACC: '
    ...
    def generate_kaplan_meier(self):
        ...
        for grp, tbl in self.merged_df.groupby('Dataset'):
            ...
            ax = kmf[grp].plot_survival_function(ax=ax, ci_alpha=0.2)
        with open(self.stats_output, 'a') as fi:
            self.dataset_kms = KaplanMeierStatistics(self.km_dataset,
                                                     'Datasets',
                                             key_1='ADNI',key_2='NACC')
        ...
        with open(self.stats_output, 'a') as fi:
            for dataset, df in self.merged_df.groupby('Dataset'):
                for col in col_list:
                    ...
                    x = [df.loc[df[grouper] == _x,col].to_numpy() for
                         _x in unique_vals]
                    ...
                    p = posthoc_dunn(sub_df, val_col=col, group_col=grouper,
                                     p_adjust="bonferroni")
                    ...
                    [fi.write('\t {}\n'.format(p[i,:])) for i in range(p.shape[
                                                                         0])]
    ...
    def retrieve_all_stats(self):
        ...
        self.get_kruskal_wallis(['MMSE', 'Age (yrs)', 'Aβ42 (pg/mL)', 't-tau (pg/mL)',
                        'p-tau (pg/mL)', 'Education (yrs)'])
        self.get_pooled_mwu(['MMSE', 'Age (yrs)', 'Education (yrs)'])
        self.print_lm(["Time to progress to AD", "#APOE \u03B54 Alleles",
                       'Sex'])
...
def _get_melted_df(df, continuous_vars,
                   id_variables):
    df = df[
        continuous_vars +
        list(id_variables)
...
def _merge_df(train, test):
    train_df = train.df.copy().rename(
            columns=train.column_maps
    )
    train_df.loc[:,'Dataset'] = 'ADNI'
    test_df = test.df.copy().rename(
            columns=test.column_maps
    )
    test_df.loc[:,'Dataset'] = 'NACC'
    new_df = pd.concat([train_df, test_df], axis=0,
                               ignore_index=True)
    new_df['Dataset'] = new_df['Dataset'].astype(
