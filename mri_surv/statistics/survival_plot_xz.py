...
from statistics.utilities import time_and_progress, CONFIG, pchip_interpolator, bootstrap_ci
from statistics.statistics_formatters import kaplan_meier_estimator
from typing import Tuple, Union
from typing_extensions import Literal
from simple_mlps.mlp_wrappers import make_struc_array
...
@pa.check_types
def load_metadata_mlp_pivot_raw() -> Tuple[MlpPivotSurvSchemaRaw, MlpPivotSurvSchemaRaw, MlpPivotSurvSchemaRaw]:
    df_long = load_mlp()
    ...
    vit_pivot = pd.read_csv('./vit/SViT_raw.csv', index_col=0).sort_values(by=['RID'], ignore_index=True)
    vit_pivot = _add_clusters_and_format_df(vit_pivot, clusters)

    return MlpPivotSurvSchemaRaw.validate(mlp_pivot), \
            MlpPivotSurvSchemaRaw.validate(cnn_pivot), \
            MlpPivotSurvSchemaRaw.validate(vit_pivot)

@pa.check_types
def _add_clusters_and_format_df(pivot_tbl: pd.DataFrame, clusters: pd.DataFrame) -> MlpPivotSurvSchemaRaw:
...
@pa.check_types
def load_metadata_mlp_pivot() -> Tuple[MlpPivotSurvSchema, MlpPivotSurvSchema, MlpPivotSurvSchema]:
    df_long = load_mlp()
    metadata = load_demographics()
    metadata = time_and_progress(metadata)
    ...
    cnn_pivot = pd.read_csv('./cgan_m/cnn/SCNN.csv', index_col=0).sort_values(by=['RID'], ignore_index=True)
    cnn_pivot['Cluster Idx'] = clusters.loc[cnn_pivot['RID'], 'Cluster Idx'].to_numpy()
    cnn_pivot['TIMES'] = cnn_pivot['TIMES'].astype(float)

    vit_pivot = pd.read_csv('./vit/SViT.csv', index_col=0).sort_values(by=['RID'], ignore_index=True)
    vit_pivot['Cluster Idx'] = clusters.loc[vit_pivot['RID'], 'Cluster Idx'].to_numpy()
    vit_pivot['TIMES'] = vit_pivot['TIMES'].astype(float)

    return MlpPivotSurvSchema.validate(mlp_pivot), MlpPivotSurvSchema.validate(cnn_pivot), MlpPivotSurvSchema.validate(vit_pivot)
...
def predict_survival_by_group_extra(df, group_variable, survival_data, name):
    for group, sub_df in df.groupby(group_variable):
        pred = sub_df[eval(CONFIG['new_bins']).astype(str)].to_numpy()
        top_pred = bootstrap_ci(pred, [0.025, 0.975], 10000)
        survival_data[group]['pred_top_'+name] = top_pred[0]
        survival_data[group]['pred_bot_'+name] = top_pred[1]
    return survival_data
...
def plot_survival_data_overlay_by_group(survival_data_dict: dict, label='NACC'):
    ...
    for key, value in survival_data_dict.items():
        ...
        polys_vit = plt.fill_between(np.arange(0,109,1), value['pred_bot_cnn'], value['pred_top_vit'], alpha=0.5, axes=ax, facecolor=cc.glasbey[step*2])
        ...
        ax.set_ylim([0, 1])
        ax.set_xlim([0, 108])
        ax.legend([polys_mlp, polys_cnn, polys_vit, line.get_lines()[0]], ['Predicted survival (MLP)','Predicted survival (CNN)', 'Predicted survival (ViT)', 'Kaplan Meier Estimate'], fontsize='large')
...
def iterate_summary_stats():
    mlp, cnn, vit = load_metadata_mlp_pivot_raw()
    pivots = {'MLP': mlp, 'CNN': cnn, 'VIT': vit}
    ...
    return pd.concat(df_collection, axis=0, ignore_index=True)

def main():
    mlp_pivot, cnn_pivot,  vit_pivot = load_metadata_mlp_pivot()
    pivots = [mlp_pivot, cnn_pivot,  vit_pivot]
